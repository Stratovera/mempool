...
ensure_fresh_network() {
    ...
    if [[ -f "${net_dir}/docker-compose.yml" && "${ALLOW_REDEPLOY:-false}" != true ]]; then
        die "Existing deployment detected for ${network} at ${net_dir} (set ALLOW_REDEPLOY=true to override)"
    fi
}
